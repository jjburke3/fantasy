            ...
            try:
                ...
                for i, game in enumerate(linkGroup):
                                ...
                                if len(row.find('th').find_all('a'))>0:
                                    ...
                                    if fumbles not in ['0',''] or fumblesLost not in ['0','']:
                                        ...
                                        try:
                                            ...
                                            print(str(e))
                                            c.execute(errorSQL % (str(e).replace("'",'_').replace('"','_'), sql.replace("'","_")))
                                            conn.commit()
                    ...
                    try:
                        ...
                        print(str(e))
                        c.execute(errorSQL % (str(e).replace("'",'_').replace('"','_'), snapSQL.replace("'","_")))
                    
                    
            except Exception as e:
                print(str(e))
                c.execute(errorSQL % (str(e).replace("'",'_').replace('"','_'), ''))
                conn.commit()
